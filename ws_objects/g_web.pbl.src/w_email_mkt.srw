$PBExportHeader$w_email_mkt.srw
forward
global type w_email_mkt from w_g_tab0
end type
type dw_xplistbar from u_dw_xplistbar within w_email_mkt
end type
type dw_mkt from uo_d_std_1 within w_email_mkt
end type
type dw_web from uo_d_std_1 within w_email_mkt
end type
type dw_xplistbar_info from u_dw_xplistbar within w_email_mkt
end type
type dw_xplistbar_trova from u_dw_trova within w_email_mkt
end type
type dw_periodo from uo_dw_periodo within w_email_mkt
end type
end forward
...
global w_email_mkt w_email_mkt
...
//				kst_tab_arfa.clie_3 = kst_tab_memo.id_memo
...
//				kst_tab_certif.clie_2 = kst_tab_memo.id_memo
...
st_tab_memo_utenti kst_tab_memo_utenti_da, kst_tab_memo_utenti_a
	...
	try
		...
		dw_lista_0.reset()  
		k_return = dw_lista_0.retrieve(ki_id, kst_tab_memo_utenti_da.stato, kst_tab_memo_utenti_a.stato, kst_tab_memo_utenti_da.x_utente, kist_tab_memo_utenti.id_sr_utente , k_data_da, k_data_a)
...
//		if isnull(NewWidth) then NewWidth = this.width
		...
		dw_xplistbar_info.Height = NewHeight - dw_xplistbar.Height - dw_xplistbar_trova.Height  // 600 //(NewHeight - dw_xplistbar.Height) / 2  
		...
		if ki_st_open_w.flag_adatta_win = KKG.ADATTA_WIN 	and not(ki_personalizza_pos_controlli) then
			...
			dw_lista_0.height = NewHeight * 0.60  //(dw_xplistbar.Height + dw_xplistbar_info.height) * 0.60 
			dw_mkt.height = NewHeight - dw_lista_0.height  - k_spess_bordi_y //(dw_xplistbar.Height + dw_xplistbar_info.height) - dw_lista_0.height  - k_spess_bordi_y
...
type st_ordina_lista from w_g_tab0`st_ordina_lista within w_email_mkt
end type

type st_aggiorna_lista from w_g_tab0`st_aggiorna_lista within w_email_mkt
end type

type cb_ritorna from w_g_tab0`cb_ritorna within w_email_mkt
end type

type st_stampa from w_g_tab0`st_stampa within w_email_mkt
end type

type cb_visualizza from w_g_tab0`cb_visualizza within w_email_mkt
end type

type cb_modifica from w_g_tab0`cb_modifica within w_email_mkt
end type

type cb_aggiorna from w_g_tab0`cb_aggiorna within w_email_mkt
end type

type cb_cancella from w_g_tab0`cb_cancella within w_email_mkt
end type

type cb_inserisci from w_g_tab0`cb_inserisci within w_email_mkt
end type

type dw_dett_0 from w_g_tab0`dw_dett_0 within w_email_mkt
integer x = 1819
integer y = 1648
integer width = 722
integer height = 248
...
type st_orizzontal from w_g_tab0`st_orizzontal within w_email_mkt
integer x = 1339
integer y = 756
integer width = 1307
...
type dw_lista_0 from w_g_tab0`dw_lista_0 within w_email_mkt
event set_titolo ( )
integer x = 992
integer y = 24
integer width = 2930
