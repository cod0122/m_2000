...
//--- Aggiorna arch di config
	...
	if kst_profilestring_ini.esito = kkg_esito.ok then
		...
		if len(trim(kst_profilestring_ini.valore)) > 0 then
			...
			k_programma = trim(kst_profilestring_ini.valore)  +  KKG.PATH_SEP + "xWxp"  +  KKG.PATH_SEP  +  "m2000." + trim(k_programma) + ".exe"
...
//--- protezione campi per impedire la modifica
	if k_operazione = "1" then  //proteggi
		k_colore=string(KKG_COLORE.GRIGIO) //rgb(192,192,192))
	else   // sproteggi
		if k_operazione = "0" then  //sproteggi
			k_colore=string(KKG_COLORE.BIANCO) //rgb(255,255,255))
		end if
	end if
...
return kguo_g.u_replace(k_str, k_char_old, k_char)
...
//--- Aggiorna arch di config
	...
	if kst_profilestring_ini.esito = kkg_esito.ok then
		...
		if len(trim(kst_profilestring_ini.valore)) > 0 then
			...
			run (trim(kst_profilestring_ini.valore) + KKG.PATH_SEP + "xWxp" + KKG.PATH_SEP + "g_upd_ver.exe", normal!)
...
Long ll_place
	...
	if ll_place > 0 then
		...
		k_return = ls_name + "," + ls_driver + "," + ls_port
	end if
