...
public function integer esporta_fatt_old (ref st_fatt_elettronica ast_fatt_elettronica) throws uo_exception
...
public function integer esporta_fatt_old (ref st_fatt_elettronica ast_fatt_elettronica) throws uo_exception;//
//--------------------------------------------------------------------------------------------------------------
//--- Esporta Fatture nel formato XML (formato PA fattura elettronica V.1.1)
//--- 
//--- Input: numero+data fattura da... a... + nome_file = path dove salvare il file delle fatture elettroniche (formato XML)
//--- out: nome_file (l'ultimo)
//--- Rit: numero fatture scritte sul file 
...
//			destroy kpbdom_proc; kpbdom_proc = create PBDOM_PROCESSINGINSTRUCTION
			...
			kpbdom_el_root.setattribute("versione", "1.1")
			kpbdom_el_root.setnamespace( "p", "http://www.fatturapa.gov.it/sdi/fatturapa/v1.1", false)  // mette la p: davanti alla <FatturaElettronica>
			kpbdom_el_root.addnamespacedeclaration( "ds", "http://www.w3.org/2000/09/xmldsig#")
			kpbdom_el_root.addnamespacedeclaration( "p", "http://www.fatturapa.gov.it/sdi/fatturapa/v1.1")
			...
			kpbdom_el_node111.addcontent("SDI11")
			kpbdom_el_node11.addcontent(kpbdom_el_node111)
...
//--- scrive il tag del foglio di stile			
			if k_byte > 0 then
				k_record = "<?xml-stylesheet type=~"text/xsl~" href=~"fatturapa_v1.1.xsl~"?>"      // richiama lo stylsheet
				k_byte = FileWriteEx(k_FileNum, k_record)
			end if		
...
//		if ast_fatt_elettronica.aggiorna = 1 then
//			kuf1_prof = create kuf_prof
//			kuf1_prof.set_fatture_esportate(kst_fatt_elettronica[])
//		end if
