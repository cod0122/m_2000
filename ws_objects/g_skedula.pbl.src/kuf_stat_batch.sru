...
public function string run_stat_u_m2000_avgtimeplant () throws uo_exception
end prototypes
...
//	end if			
	...
	if k_rc < 0 then
		...
		kst_esito.sqlcode = -1
		kst_esito.sqlerrtext = "Errrore in Generazione Statistici: '" &
...
//--- lancio spl (datastore)
	...
	if k_rc < 0 then
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.sqlcode = -1
		kst_esito.sqlerrtext = "Errrore in Generazione Statistici Tempi Medi Giri Impianto: '" &
									  + k_esito + "': esito " + string (k_rc) 
	else
		if k_rc = 0 then
			kst_esito.esito = kkg_esito.ko
			kst_esito.sqlcode = 0
			kst_esito.sqlerrtext = "Anomalia in generazione Statistici Tempi Medi Giri Impianto ' " &
										  + k_esito + " Nessun dato estratto! "
		else
			kst_esito.esito = kkg_esito.ok
			kst_esito.sqlcode = 0
			kst_esito.sqlerrtext = "Generazione Statistici Tempi Medi Giri Impianto terminata correttamente: " + k_esito
