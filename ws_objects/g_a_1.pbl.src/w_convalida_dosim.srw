$PBExportHeader$w_convalida_dosim.srw
forward
...
global w_convalida_dosim w_convalida_dosim
...
////-----------------------------------------------------------------------------------------------------------------------
//	try
////--- Genera avviso Pronto Merce per poi fare l'Invio via e-mail 
...
//--- Aggiorna dati di convalida sul Riferimento MECA 
		...
		kst_tab_meca.st_tab_g_0.esegui_commit = "NO"
		kiuf_meca_dosim.set_convalida(kst_tab_meca)    // AGGIORNA MECA_DOSIM
...
kuf_utility kuf1_utility
	...
	try
		...
		if LenA(trim(kst_tab_meca.err_lav_ok)) = 0 or isnull(kst_tab_meca.err_lav_ok)  or kst_tab_meca.err_lav_ok = kuf_meca_dosim.ki_err_lav_ok_da_conv then
...
//--- attraverso i parametri di INPUT piglia il ID_MECA
	...
	if tab_1.tabpage_1.dw_1.rowcount( ) = 0 then 
		...
		if ki_st_open_w.flag_modalita = kkg_flag_modalita.inserimento or kst_tab_meca_dosim.id_meca = 0 then
...
//--- impostazioni generali dei dati window
			...
			tab_1.tabpage_1.dw_1.setrow(k_ctr)
			if kst_tab_meca.st_tab_meca_dosim.dosim_lotto_dosim > " " then
				tab_1.tabpage_1.dw_1.setcolumn("num_int")
			else
				tab_1.tabpage_1.dw_1.setcolumn("dosim_lotto_dosim")
			end if
			tab_1.tabpage_1.dw_1.resetupdate()
...
//--- Imposta dw dal DB dosimetrie
		leggi_db_to_dw(kst_tab_meca_dosim) 

//--- recupero eventuale MEMO
		kst_tab_meca_memo.id_meca = kst_tab_meca_dosim.id_meca
		if kst_tab_meca_memo.id_meca > 0 then
			kuf1_armo_inout = create kuf_armo_inout
//			kst_tab_meca_memo.tipo_sv = ki_st_open_w.sr_settore  //kuf1_armo_inout.kki_tipo_sv_dsm
			kuf1_armo_inout.get_id_meca_memo_max(kst_tab_meca_memo)
			tab_1.tabpage_1.dw_1.setitem(1, "id_meca_memo", kst_tab_meca_memo.id_meca_memo)
		else
			tab_1.tabpage_1.dw_1.setitem(1, "id_meca_memo", 0)
		end if	
		
		ki_st_open_w.flag_modalita = kkg_flag_modalita.modifica
		kst_tab_meca.err_lav_ok = tab_1.tabpage_1.dw_1.getitemstring(1, "err_lav_ok")
		
		if isnull(kst_tab_meca.err_lav_ok) or kst_tab_meca.err_lav_ok = kuf_meca_dosim.ki_err_lav_ok_da_conv then

		//--- Se data a zero la imposto con data oggi
			if kst_tab_meca.st_tab_meca_dosim.dosim_data = date(0) or isnull(kst_tab_meca.st_tab_meca_dosim.dosim_data) then
				kst_tab_meca.st_tab_meca_dosim.dosim_data = kguo_g.get_dataoggi()
			end if
			
		//--- Se anno a zero lo imposto con anno di data oggi
			if tab_1.tabpage_1.dw_1.getitemdate(1, "data_int") = date(0) &
				or isnull(tab_1.tabpage_1.dw_1.getitemdate(1, "data_int")) then
				tab_1.tabpage_1.dw_1.setitem(1, "data_int", date(string(kguo_g.get_anno()) +"-01-01"))
			end if
...
//		kist_tab_meca.id = tab_1.tabpage_1.dw_1.getitemnumber(1, "id_meca")
		...
		if isnull(kist_tab_meca.err_lav_ok) then
			kist_tab_meca.err_lav_ok = kuf_meca_dosim.ki_err_lav_ok_da_conv
		end if
...
//02082007--- Messaggio dello stato di Lavorazione dei Lotti, avverto se non completo
	if kist_tab_meca.id > 0 then
		...
		kst_tab_armo.colli_2 = kiuf_armo.get_colli_lotto( kst_tab_armo ) // giugno.2016 xbcode( kst_tab_armo ) //270614 get_colli_entrati( kst_tab_armo )
		//if kst_esito.esito = kkg_esito.ok then
		if kst_tab_armo.colli_2 > 0 then
			...
			if kst1_tab_armo.colli_2 > 0 then
				...
				kst2_tab_armo.colli_2 = kiuf_armo.get_colli_danontrattare( kst2_tab_armo )   //giugno.2016 get_colli_da_non_trattare( kst2_tab_armo )
...
//--- personalizza colore della dw a seconda della stato
	if tab_1.tabpage_1.dw_1.rowcount() > 0 then
		...
		choose case ki_err_lav_ok
			...
			case "", kuf_meca_dosim.ki_err_lav_ok_da_conv
				tab_1.tabpage_1.dw_1.modify("DataWindow.Color='" + string(kkg_colore.BLU_CHIARO)+"'")

			case kuf_meca_dosim.ki_err_lav_ok_conv_da_aut
			...
			case kuf_meca_dosim.ki_err_lav_ok_conv_ko_bloc
			...
			case kuf_meca_dosim.ki_err_lav_ok_conv_ko_sbloc
				tab_1.tabpage_1.dw_1.modify("DataWindow.Color='" + string(kkg_colore.GRIGIO)+"'")
				ki_st_open_w.flag_modalita = kkg_flag_modalita.visualizzazione
				tab_1.tabpage_1.dw_1.modify("b_ok.visible='0'")
...
//--- Inabilita campi alla modifica se Visualizzazione
	...
	if trim(ki_st_open_w.flag_modalita) = kkg_flag_modalita.visualizzazione &
		...
		or kist_tab_meca.err_lav_ok = kuf_meca_dosim.ki_err_lav_ok_conv_ko_bloc then
...
//	if kist_memo.st_tab_memo.id_memo  > 0 then
		...
		if k_risposta_load_memo_link = 3 then
			...
			tab_1.tabpage_1.dw_1.setitem( 1, "id_meca_memo", 1)  // forza il cambiamento dell'icona
...
if k_file_nr > 0 then	
	...
	if tab_1.tabpage_1.dw_1.getitemnumber(1, "id_meca") > 0 then
		...
		messagebox("Operazione fermata", "Prima si deve indicare il numero del Lotto", stopsign!) 
	end if
end if
...
type st_ordina_lista from w_g_tab_3`st_ordina_lista within w_convalida_dosim
end type

type st_aggiorna_lista from w_g_tab_3`st_aggiorna_lista within w_convalida_dosim
end type

type cb_ritorna from w_g_tab_3`cb_ritorna within w_convalida_dosim
integer x = 1522
integer y = 1784
end type

type st_stampa from w_g_tab_3`st_stampa within w_convalida_dosim
integer x = 997
integer y = 1704
end type

type cb_visualizza from w_g_tab_3`cb_visualizza within w_convalida_dosim
integer x = 1915
integer y = 1792
end type

type cb_modifica from w_g_tab_3`cb_modifica within w_convalida_dosim
integer x = 398
integer y = 1784
end type

type cb_aggiorna from w_g_tab_3`cb_aggiorna within w_convalida_dosim
integer x = 782
integer y = 1784
end type

type cb_cancella from w_g_tab_3`cb_cancella within w_convalida_dosim
integer x = 1152
integer y = 1784
end type

type cb_inserisci from w_g_tab_3`cb_inserisci within w_convalida_dosim
integer x = 18
integer y = 1788
end type
...
if kidw_selezionata.dataobject = "d_convalida_dosim" then
...
choose case dwo.name
	...
	//--- popolo il datasore (dw non visuale) per appoggio elenco
		...
		choose case dwo.name
			...
			case "b_meca"
				kuf1_armo = create kuf_armo
				if trim(ki_err_lav_ok) = trim(kuf_meca_dosim.ki_err_lav_ok_da_conv) then
