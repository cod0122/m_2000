...
public function long of_setbranches (readonly datawindow a_dataobject, long al_row)
public function long of_setbranches (readonly datastore a_dataobject, long al_row)
public function integer of_adjustsize (ref integer al_y)
...
ll_start = Cpu()
//iw_parent.event dynamic ue_testmessage('of_getvalues START*** '+string(idw_dw.rowcount())) //testing!!, event will fail silently if doesn't exist?
...
IF is_ColType = 'date' THEN
	...
	ls_return = dw_powerfilter.modify( 't_expand.text="'+is_Expand+'"')
...
END IF
of_setfont( is_FontName, is_FontColor)
...
FOR ll_row = 1 to ll_rowcount
	CHOOSE CASE is_ColType
		...
		CASE ELSE
			...
			RETURN
	END CHOOSE
	IF Cpu() - ll_start > il_MaxTime THEN
//	IF ll_dropdownrows > il_MaxItems THEN
		//d7f9 new code 2/1/2013
		...
		//iw_parent.event dynamic ue_testmessage('Rows Examined: ' + string(ll_row)) //testing!!, event will fail silently if doesn't exist?
		//iw_parent.event dynamic ue_testmessage('Total Rows: ' + string(ll_rowcount)) //testing!!, event will fail silently if doesn't exist?
		EXIT //limit dropdown to certain number of items (Excel limits to 10,000)
	END IF
NEXT

//iw_parent.event dynamic ue_testmessage('StartSorts, elapsed cpu secs: ' + string(Cpu() - ll_start)) //testing!!, event will fail silently if doesn't exist?
...
//	st_bottomsizer.y = this.height - st_bottomsizer.height
//	st_sidesizer.x = this.width - st_sidesizer.width
...
////enable and position date search options dropdown
//choose case upper(is_ColType)
//	case 'DATE','DATET'
//		dw_datesearchopts.visible=true
//		p_search.x = dw_datesearchopts.x - p_search.width
//	case else
//		dw_datesearchopts.visible=false
//		p_search.x = dw_datesearchopts.x
//end choose
...
//need to check for full/partial/empty years and months to set checkyear and checkmonth appropriately
//IF dw_powerfilter.dataobject = 'd_powerfilter_datetree' THEN
//	ids_powerfilter[ii_colnum].setsort('dateitem a')
//ELSEIF dw_powerfilter.dataobject = 'd_powerfilter_datetimetree' THEN
...
Integer	li_setvalue
Long ll_row,ll_rows,ll_filteredrows
//iw_parent.event dynamic ue_testmessage('of_customfilter START')
IF as_filter1 <> '' THEN 
...
iu_powerfilter_checkbox.of_buildfilter(is_ColumnFilter,ii_ColNum)
//iw_parent.event dynamic ue_testmessage('of_customfilter END')
RETURN 0
...
//// Set the start equal to the current CPU usage.
//ll_start = Cpu()
//iw_parent.event dynamic ue_testmessage('*** Begin of_getvaluesoriginal *** '+string(idw_dw.rowcount())) //testing!!, event will fail silently if doesn't exist?
//// Executable statements being timed follow
//
//SetPointer ( HourGlass! )
//
...
//// that were used (current CPU time - start).
//ll_used = Cpu() - ll_start
//iw_parent.event dynamic ue_testmessage('*** End of_getvaluesoriginal, cpu secs used: ' + string(ll_used)) //testing!!, event will fail silently if doesn't exist?
//
//
end subroutine
...
public function long of_setbranches (readonly datawindow a_dataobject, long al_row);/*
This function travels down a datetree datawindow and sets the year and month (branch) checks as on/off/partial depending on the values of the days (leaves)

return value is last row of the same month as the row passed in, so if needed, can increment and call 

*/
Long	ll_row,ll_rows,ll_firstrow,ll_lastrow,ll_rowcount
Integer	li_year,li_month
Boolean	ib_partial
String	ls_colname

ll_row = al_row // start at the top

CHOOSE CASE is_coltype 
	CASE 'date'
		ls_colname = 'dateitem'
		li_year = year(a_dataobject.GetItemDate(ll_row,ls_colname)) //reference year for row/band selected
		li_month = month(a_dataobject.GetItemDate(ll_row,ls_colname)) //reference month for row/band selected
	CASE 'datet'
		ls_colname = 'datetimeitem'
		li_year = year(date(a_dataobject.GetItemDateTime(ll_row,ls_colname))) //reference year for row/band selected
		li_month = month(date(a_dataobject.GetItemDateTime(ll_row,ls_colname))) //reference month for row/band selected
	CASE ELSE
		//SHOULDN'T RUN THIS COD
		RETURN -1
END CHOOSE

IF isnull(li_year) THEN //(Blank) row, only 1 row, set month and year, then get out
	a_dataobject.setitem(ll_row,'checkyear',a_dataobject.GetItemNumber(ll_row,'check'))
	a_dataobject.setitem(ll_row,'checkmonth',a_dataobject.GetItemNumber(ll_row,'check'))
	RETURN ll_row
END IF

ll_rowcount = a_dataobject.rowcount()

ll_firstrow	= a_dataobject.Find ( "year("+ls_colname+") = "+string(li_year),1,ll_rowcount)
ll_lastrow	= a_dataobject.Find ( "year("+ls_colname+") = "+string(li_year),ll_rowcount,ll_firstrow)
if ll_lastrow = 0 then ll_lastrow = ll_firstrow 
ib_partial = FALSE
FOR ll_row = ll_firstrow +1 TO ll_lastrow
	IF a_dataobject.GetItemNumber(ll_row,'check') <> a_dataobject.GetItemNumber(ll_row - 1,'check') THEN
		ib_partial = TRUE
		EXIT
	END IF
NEXT
IF ib_partial THEN 
	a_dataobject.setitem(ll_firstrow,'checkyear',2)
ELSE
	a_dataobject.setitem(ll_firstrow,'checkyear',a_dataobject.GetItemNumber(ll_firstrow,'check'))
END IF

ll_firstrow	= a_dataobject.Find ( "year("+ls_colname+") = "+string(li_year)+" and month("+ls_colname+") = "+string(li_month),1,ll_rowcount)
ll_lastrow	= a_dataobject.Find ( "year("+ls_colname+") = "+string(li_year)+" and month("+ls_colname+") = "+string(li_month),ll_rowcount,ll_firstrow)
...
NEXT
IF ib_partial THEN 
	a_dataobject.setitem(ll_firstrow,'checkmonth',2)
ELSE
	a_dataobject.setitem(ll_firstrow,'checkmonth',a_dataobject.GetItemNumber(ll_firstrow,'check'))
END IF
//done setting the year and month for a particular starting row.
return ll_lastrow
end function
...
cbx_1.setfocus() //fix graphic glitch?
...
IF ll_rows > 0 THEN
	...
	if Len(ls_val) = 0 then
		...
		//check to see if date tree and reset the branches from the original settings which have been restored
		choose case is_coltype
			case 'date','datet'
				FOR ll_row = 1 to ll_rows
					ll_row = of_setbranches(dw_powerfilter,ll_row)
				NEXT
		end choose
	else
	...
	//if date/datetime column, expand treeview here
	choose case is_coltype
		case 'date','datet'
...
SetPointer(HourGlass!)
//iw_parent.event dynamic ue_testmessage('OK Clicked')
...
this.setfocus()
//this.setrow(row) don't want to setrow, causes problems with datetree row selection
iu_powerfilter_monthopts.visible = FALSE
iu_powerfilter_predeffilters.visible = FALSE
...
.	08/03/2010	1.00		Reesejr	Initial Version
.============================================================*/
String 	ls_band,ls_level
Integer	li_tab,li_year,li_month,li_checked,li_setvalue
Boolean	ib_partial
Long		ll_clickedrow,ll_row,ll_firstrow,ll_lastrow
...
CHOOSE CASE dwo.name 
	CASE 't_expand'
		...
		RETURN
//	CASE 'b_expand'
//		RETURN // handle buttonclick in buttonclick event, don't perform click event logic
END CHOOSE
...
//	iw_parent.event dynamic ue_testmessage('ls_band: '+ls_band) //testing!!, event will fail silently if doesn't exist?
	li_tab = Pos(ls_band, "~t", 1)
	ls_level = Left(ls_band, li_tab - 1)
//	iw_parent.event dynamic ue_testmessage('ls_level: '+ls_level) //testing!!, event will fail silently if doesn't exist?
	IF ls_level = 'header' THEN RETURN //
	ll_clickedrow = long(Mid(ls_band, li_tab + 1))
	IF ls_level = 'detail' and ll_clickedrow = 0 THEN RETURN 
	if ll_clickedrow > 0 THEN
		//
	ELSE
		IF ll_clickedrow > 0 THEN
			ll_clickedrow = row //might be using keyboard instead of mouse?
		ELSE
			ll_clickedrow = 1
		END IF
	END IF
	li_year = year(this.GetItemDate(ll_clickedrow,'dateitem')) //reference year for row/band selected
	li_month = month(this.GetItemDate(ll_clickedrow,'dateitem')) //reference month for row/band selected
	IF isnull(li_year) THEN //(Blank) row, so set that one row and be done
		li_checked = this.GetItemNumber(ll_clickedrow,'checkyear') //if 0 or 2, set all year to 1, if 1, set all year to 0
		CHOOSE CASE li_checked
			CASE 0,2
				li_setvalue = 1
			CASE 1
				li_setvalue = 0
		END CHOOSE
		this.SetItem(ll_clickedrow,'checkyear',li_setvalue)
		this.SetItem(ll_clickedrow,'checkmonth',li_setvalue)
		this.SetItem(ll_clickedrow,'check',li_setvalue)				
	ELSE
		CHOOSE CASE ls_level
			CASE 'tree.level.1' // = Year clicked 
				...
				CHOOSE CASE li_checked
					...
					CASE 1
						li_setvalue = 0
				END CHOOSE
				ll_row = ll_clickedrow
				DO While year(this.GetItemDate(ll_row,'dateitem')) = li_year
					this.SetItem(ll_row,'checkyear',li_setvalue)
					this.SetItem(ll_row,'checkmonth',li_setvalue)
					this.SetItem(ll_row,'check',li_setvalue)
					ll_row ++
					IF ll_row > this.rowcount() then exit
				LOOP
			CASE 'tree.level.2' // = Month clicked
				li_checked = this.GetItemNumber(ll_clickedrow,'checkmonth') //if 0 or 2, set all month to 1, if 1, set all month to 0
				CHOOSE CASE li_checked
					CASE 0,2
						li_setvalue = 1
					CASE 1
			...
			CASE ELSE
				...
				IF this.GetItemNumber(ll_clickedrow,"check") = 1 THEN
					this.SetItem( ll_clickedrow, "check", 0)
				ELSE
					this.SetItem( ll_clickedrow, "check", 1)
				END IF
				this.SetRow( ll_clickedrow )
		END CHOOSE
	END IF
	//need to check for full/partial/empty years and months to set checkyear and checkmonth appropriately
	...
	of_setbranches(this,ll_clickedrow)
...
ELSEIF this.dataobject = 'd_powerfilter_datetimetree' THEN
	...
	ELSE
		...
		END IF
	END IF
	li_year = year(date(this.GetItemDatetime(ll_clickedrow,'datetimeitem'))) //reference year for row/band selected
	li_month = month(date(this.GetItemDatetime(ll_clickedrow,'datetimeitem'))) //reference month for row/band selected
	IF isnull(li_year) THEN //(Blank) row, so set that one row and be done
		li_checked = this.GetItemNumber(ll_clickedrow,'checkyear') //if 0 or 2, set all year to 1, if 1, set all year to 0
		CHOOSE CASE li_checked
			CASE 0,2
