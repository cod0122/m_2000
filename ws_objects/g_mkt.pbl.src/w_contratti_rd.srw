$PBExportHeader$w_contratti_rd.srw
forward
global type w_contratti_rd from w_g_tab_3
end type
...
integer width = 3447
integer height = 1072
string title = "Contratto Studio e Sviluppo"
boolean ki_toolbar_window_presente = true
boolean ki_fai_nuovo_dopo_update = false
end type
global w_contratti_rd w_contratti_rd

type variables
//
protected kuf_contratti_rd kiuf_contratti_rd
protected kuf_clienti kiuf_clienti
protected st_tab_contratti_rd kist_tab_contratti_rd //memorizzare i dati di entrata
...
public subroutine u_set_imp_contratto (st_tab_contratti_rd kst_tab_contratti_rd)
public subroutine u_set_imp_acconto (st_tab_contratti_rd kst_tab_contratti_rd)
public function st_tab_contratti_rd u_calcola_tot ()
...
//if k_riga > 0 then
//
//	kiuf_contratti_rd.if_isnull( kst_tab_listino)
//	
//end if
//
...
//--- Se nuova riga Imposto il campo Contatore ID (SERIAL)				
					if k_new_rec then
						kiuf_contratti_rd.get_ultimo_id(kst_tab_contratti_rd)
						tab_1.tabpage_1.dw_1.setitem(1, "contratti_rd_id_contratto_rd", kst_tab_contratti_rd.id_contratto_rd)
...
choose case tab_1.selectedtab 
	case 1 
		...
		k_riga = tab_1.tabpage_1.dw_1.rowcount()	
		if k_riga > 0 then
			kst_tab_contratti_rd.id_contratto_rd = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "contratti_rd_id_contratto_rd")
		...
		if k_riga > 0 and kst_tab_contratti_rd.id_contratto_rd > 0 then	
		...
		//=== Richiesta di conferma della eliminazione del rek
			...
			if messagebox("Elimina Contratto Studio Sviluppo", "Sei sicuro di voler Cancellare : ~n~r" + &
						string(kst_tab_contratti_rd.id_contratto_rd, "####0") + " di " + string(kst_tab_clienti.codice) &
		...
		//=== Cancella la riga dal data windows di lista
				...
				messagebox("Elimina Contratto Studio Sviluppo", "Operazione Annullata !!")
			end if
...
//--- Controllo il primo tab
	if tab_1.tabpage_1.dw_1.rowcount() > 0 then
		...
		kst_esito = kiuf_contratti_rd.u_check_dati(kds_inp)
	end if
...
st_tab_contratti_rd kst_tab_contratti_rd
	...
	if k_ctr > 0 then
		...
		kst_tab_contratti_rd.id_contratto_rd = tab_1.tabpage_1.dw_1.getitemnumber(k_ctr, "contratti_rd_id_contratto_rd")
...
//=== Se non sono in caricamento allora prelevo l'ID dalla dw
		...
		if isnull(kst_tab_contratti_rd.id_contratto_rd) or kst_tab_contratti_rd.id_contratto_rd = 0 then				
			tab_1.tabpage_1.dw_1.setitem(k_ctr, "contratti_rd_id_contratto_rd", 0)
		end if
...
//--- Se NO inserimento.... 
		k_rc = tab_1.tabpage_1.dw_1.retrieve( kist_tab_contratti_rd.id_contratto_rd ) 
		...
		choose case k_rc
			...
			case is < 0				
				messagebox("Operazione fallita", &
					...
					"(ID Contratto Studio e Svulippo cercato :" + trim(string(kist_tab_contratti_rd.id_contratto_rd)) + ")~n~r" )
			...
			case 0
				...
				messagebox("Ricerca fallita", &
					...
					"(ID cercato: "  &
					 + trim(string(kist_tab_contratti_rd.id_contratto_rd)) + ")~n~r" )
...
//--- se Documento già ACCETTATO/STAMPATO allora ATTENZIONE alle Modifiche	!!!!!	
					if tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_STATO_stampato then
						...
						kuo_exception.setmessage(  &
							...
							"(ID Documento cercato:" + string(kist_tab_contratti_rd.id_contratto_rd) + ") " )
						kuo_exception.messaggio_utente( )	
					else
//--- se lo STATO è ACCETTATO o NON/TRASFERITO oppure contratto ANNULLATO allora permetto solo visualizzazione						
						if tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_STATO_accettato &
								or tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_stato_respinto &
								or tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_stato_trasferito then
							ki_st_open_w.flag_modalita = kkg_flag_modalita.visualizzazione
						end if
//						if tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_STATO_accettato then
//							ki_st_open_w.flag_modalita = kkg_flag_modalita.visualizzazione
//						end if
					end if
...
//--- legge tiutte le DWC			
			...
			if tab_1.tabpage_1.dw_1.rowcount() > 0 then
				...
				k_riga = 1
				kst_tab_contratti_rd.magazzino = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "magazzino")
				kst_tab_contratti_rd.anno = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "anno")
				kst_tab_contratti_rd.offerta_data = tab_1.tabpage_1.dw_1.getitemdate(k_riga, "offerta_data")
				kst_tab_contratti_rd.offerta_validita = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "offerta_validita")
				kst_tab_contratti_rd.stampa_tradotta = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "stampa_tradotta")
				kst_tab_contratti_rd.oggetto = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "oggetto")
				kst_tab_contratti_rd.id_clie_settore = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "id_clie_settore")
				kst_tab_clie_settori.descr = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "clie_settori_descr")
				kst_tab_contratti_rd.gruppo = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "gruppo")
				kst_tab_contratti_rd.art = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "art")
				kst_tab_contratti_rd.data_inizio = tab_1.tabpage_1.dw_1.getitemdate(k_riga, "data_inizio")
				kst_tab_contratti_rd.data_fine = tab_1.tabpage_1.dw_1.getitemdate(k_riga, "data_fine")
				kst_tab_contratti_rd.id_listino_pregruppo = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_pregruppo")
				kst_tab_contratti_rd.id_listino_voce_1 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_1")
				kst_tab_contratti_rd.id_listino_voce_2 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_2")
				kst_tab_contratti_rd.id_listino_voce_3 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_3")
				kst_tab_contratti_rd.id_listino_voce_4 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_4")
				kst_tab_contratti_rd.id_listino_voce_5 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_5")
				kst_tab_contratti_rd.id_listino_voce_6 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_6")
				kst_tab_contratti_rd.id_listino_voce_7 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_7")
				kst_tab_contratti_rd.id_listino_voce_8 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_8")
				kst_tab_contratti_rd.id_listino_voce_9 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_9")
				kst_tab_contratti_rd.id_listino_voce_10 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "id_listino_voce_10")
//				kst_tab_contratti_rd.fase_b1 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_b1")
//				kst_tab_contratti_rd.fase_b2 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_b2")
//				kst_tab_contratti_rd.fase_b3 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_b3")
//				kst_tab_contratti_rd.fase_b4 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_b4")
//				kst_tab_contratti_rd.fase_b5 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_b5")
//				kst_tab_contratti_rd.fase_b6 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_b6")
//				kst_tab_contratti_rd.fase_d1 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_d1")
//				kst_tab_contratti_rd.fase_d2 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_d2")
//				kst_tab_contratti_rd.fase_d3 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_d3")
//				kst_tab_contratti_rd.fase_d4 = tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "fase_d4")
			
				tab_1.tabpage_1.dw_1.reset() 
			else
//--- piglia i dati di default dall'ultimo caricamento dell'anno in corso.... 				
				kst_tab_contratti_rd.anno = year(kg_dataoggi)
				kiuf_contratti_rd.get_dati_default( kst_tab_contratti_rd )
				if kst_tab_contratti_rd.id_contratto_rd > 0 then // se non trovo nulla allora riprovo con anno meno 1
				else
					kst_tab_contratti_rd.anno = year(kg_dataoggi) - 1
					kiuf_contratti_rd.get_dati_default( kst_tab_contratti_rd )
					if kst_tab_contratti_rd.anno = 0 then // se non trovo nulla allora riprovo con anno meno 1
						kst_tab_contratti_rd.anno = year(kg_dataoggi) 
					end if				
				end if			
			end if
...
//--- data redazione 			
			if kst_tab_contratti_rd.anno > 0 then
				tab_1.tabpage_1.dw_1.setitem(1, "anno", kst_tab_contratti_rd.anno)
			else
				tab_1.tabpage_1.dw_1.setitem(1, "anno", year(kg_dataoggi))
			end if
			if kst_tab_contratti_rd.offerta_data > date(0) then
				tab_1.tabpage_1.dw_1.setitem(1, "offerta_data", kst_tab_contratti_rd.offerta_data)
			else
				tab_1.tabpage_1.dw_1.setitem(1, "offerta_data", kg_dataoggi)
			end if
			if len(trim(kst_tab_contratti_rd.offerta_validita)) > 0 then
				tab_1.tabpage_1.dw_1.setitem(1, "offerta_validita", kst_tab_contratti_rd.offerta_validita)
			else
				tab_1.tabpage_1.dw_1.setitem(1, "offerta_validita", "")
			end if
			if kst_tab_contratti_rd.magazzino > 0 then
				tab_1.tabpage_1.dw_1.setitem(1, "magazzino", kst_tab_contratti_rd.magazzino)
			else
				tab_1.tabpage_1.dw_1.setitem(1, "magazzino", kkg_magazzino.rd )
			end if
			tab_1.tabpage_1.dw_1.setitem(1, "stato", kst_tab_contratti_rd.stato)
			tab_1.tabpage_1.dw_1.setitem(1, "data_stampa", kst_tab_contratti_rd.data_stampa)
			tab_1.tabpage_1.dw_1.setitem(1, "stampa_tradotta", kst_tab_contratti_rd.stampa_tradotta)

//--- Imposta il periodo di default			
			if kst_tab_contratti_rd.data_inizio > date(0) then
				tab_1.tabpage_1.dw_1.setitem(1, "data_inizio", kst_tab_contratti_rd.data_inizio)
			else
				tab_1.tabpage_1.dw_1.setitem(1, "data_inizio", kg_dataoggi)
			end if
			if kst_tab_contratti_rd.data_fine > date(0) then
				tab_1.tabpage_1.dw_1.setitem(1, "data_fine", kst_tab_contratti_rd.data_inizio)
			else
				tab_1.tabpage_1.dw_1.setitem(1, "data_fine", date(year(kg_dataoggi), 12, 31))
			end if
			
//--- popolo campi cliente se passato come argomento
			if kist_tab_contratti_rd.id_cliente > 0 then
...
//				end if	
				...
				kst_tab_clienti.codice = kist_tab_contratti_rd.id_cliente
				get_dati_cliente(kst_tab_clienti)
				put_video_cliente(kst_tab_clienti)
				
//				k_riga = Kdwc_clie_des.find("id_cliente="+trim(string(kist_tab_contratti_rd.id_cliente)),0,Kdwc_clie_des.rowcount())
//				if k_riga > 0 then
//					k_rc = tab_1.tabpage_1.dw_1.setitem(1, "id_cliente",  kist_tab_contratti_rd.id_cliente)
...
//--- legge dwc dei contatti del cliente
				...
				Kdwc_clie_contatti.retrieve(kist_tab_contratti_rd.id_cliente)
				if Kdwc_clie_contatti.rowcount( ) > 0 then
					k_riga = Kdwc_clie_contatti.find("id_cliente="+trim(string(kist_tab_contratti_rd.id_cliente)),0,Kdwc_clie_contatti.rowcount())
					if k_riga = 0 then
...
//--- legge dwc Oggetti già utilizzati
			if len(trim(kst_tab_contratti_rd.oggetto )) > 0 then
				k_rc = tab_1.tabpage_1.dw_1.setitem(1, "oggetto", kst_tab_contratti_rd.oggetto  )
			else
...
//--- legge dwc Settori
			if len(trim(kst_tab_contratti_rd.id_clie_settore )) > 0 then
				k_rc = tab_1.tabpage_1.dw_1.setitem(1, "id_clie_settore", kst_tab_contratti_rd.id_clie_settore  )
...
//--- Attivo dw archivio Gruppi
			kist_tab_contratti_rd.id_clie_settore = tab_1.tabpage_1.dw_1.getitemstring(1, "id_clie_settore")
			if len(trim(kist_tab_contratti_rd.id_clie_settore)) > 0 then
				k_rc = tab_1.tabpage_1.dw_1.getchild("gruppo", kdwc_gru)
				k_rc = kdwc_gru.settransobject(sqlca)
				k_rc = kdwc_gru.retrieve(kist_tab_contratti_rd.id_clie_settore)
				kdwc_gru.insertrow(0)
			end if
			
//--- Imposta Prezzi	
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_pregruppo",kst_tab_contratti_rd.id_listino_pregruppo)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_1",kst_tab_contratti_rd.id_listino_voce_1)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_2",kst_tab_contratti_rd.id_listino_voce_2)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_3",kst_tab_contratti_rd.id_listino_voce_3)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_4",kst_tab_contratti_rd.id_listino_voce_4)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_5",kst_tab_contratti_rd.id_listino_voce_5)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_6",kst_tab_contratti_rd.id_listino_voce_6)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_7",kst_tab_contratti_rd.id_listino_voce_7)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_8",kst_tab_contratti_rd.id_listino_voce_8)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_9",kst_tab_contratti_rd.id_listino_voce_9)
			tab_1.tabpage_1.dw_1.setitem(k_riga, "id_listino_voce_10",kst_tab_contratti_rd.id_listino_voce_10)
//			if kst_tab_contratti_rd.fase_b1 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b1", kst_tab_contratti_rd.fase_b1)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b1", 0)
//			end if
//			if kst_tab_contratti_rd.fase_b2 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b2", kst_tab_contratti_rd.fase_b2)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b2", 0)
//			end if
//			if kst_tab_contratti_rd.fase_b3 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b3", kst_tab_contratti_rd.fase_b3)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b3", 0)
//			end if
//			if kst_tab_contratti_rd.fase_b4 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b4", kst_tab_contratti_rd.fase_b4)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b4", 0)
//			end if
//			if kst_tab_contratti_rd.fase_b5 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b5", kst_tab_contratti_rd.fase_b5)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b5", 0)
//			end if
//			if kst_tab_contratti_rd.fase_b6 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b6", kst_tab_contratti_rd.fase_b6)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_b6", 0)
//			end if
//			if kst_tab_contratti_rd.fase_d1 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d1", kst_tab_contratti_rd.fase_d1)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d1", 0)
//			end if
//			if kst_tab_contratti_rd.fase_d2 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d2", kst_tab_contratti_rd.fase_d2)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d2", 0)
//			end if
//			if kst_tab_contratti_rd.fase_d3 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d3", kst_tab_contratti_rd.fase_d3)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d3", 0)
//			end if
//			if kst_tab_contratti_rd.fase_d4 > 0 then
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d4", kst_tab_contratti_rd.fase_d4)
//			else
//				tab_1.tabpage_1.dw_1.setitem(1, "fase_d4", 0)
//			end if
...
//--- Acquisisce i dati da passati in Argomento
	if LenA(trim(ki_st_open_w.key1)) = 0 then
		kist_tab_contratti_rd.id_contratto_rd = 0
	else
		kist_tab_contratti_rd.id_contratto_rd = long(trim(ki_st_open_w.key1))
	end if
	if LenA(trim(ki_st_open_w.key2)) = 0 then
		kist_tab_contratti_rd.id_cliente = 0
	else
		kist_tab_contratti_rd.id_cliente = long(trim(ki_st_open_w.key2))
	end if
...
//--- Inabilita campi alla modifica se Visualizzazione
		...
		if trim(ki_st_open_w.flag_modalita) <> kkg_flag_modalita.inserimento and trim(ki_st_open_w.flag_modalita) <> kkg_flag_modalita.modifica &
			 			or (tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") <> kiuf_contratti_rd.kki_STATO_riaperto and tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") <> kiuf_contratti_rd.kki_STATO_nuovo) then
		
//--- Protezione tutti i campi 
			kuf1_utility.u_proteggi_dw("1", 0, tab_1.tabpage_1.dw_1)
			kuf1_utility.u_proteggi_dw("1", "contratti_rd_id_contratto_rd", tab_1.tabpage_1.dw_1)
...
//--- se sono in MODIFICA e Contratto già stampa definitiva allora inabilito tutti i campi all'infuori che lo Stato			
			if ki_st_open_w.flag_modalita = kkg_flag_modalita.modifica &
			 			and (tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_STATO_stampato &
						         or tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_STATO_accettato &
						         or tab_1.tabpage_1.dw_1.getitemstring( 1, "stato") = kiuf_contratti_rd.kki_STATO_respinto) then
...
st_tab_contratti_rd kst_tab_contratti_rd
	...
	kst_tab_contratti_rd.voce_qta_1 = tab_1.tabpage_1.dw_1.getitemnumber(1, "voce_qta_" + a_riga_imp)
	if kst_tab_contratti_rd.voce_qta_1 > 0 then
		kst_tab_contratti_rd.voce_prezzo_1 = tab_1.tabpage_1.dw_1.getitemnumber(1, "voce_prezzo_" + a_riga_imp)
		if kst_tab_contratti_rd.voce_prezzo_1 <> 0 then
			kst_tab_contratti_rd.voce_prezzo_tot_1 = kst_tab_contratti_rd.voce_qta_1 * kst_tab_contratti_rd.voce_prezzo_1
		end if
	else
		kst_tab_contratti_rd.voce_prezzo_tot_1 = 0
	end if
	tab_1.tabpage_1.dw_1.setitem(1, "voce_prezzo_tot_" + a_riga_imp, kst_tab_contratti_rd.voce_prezzo_tot_1)

end subroutine

public subroutine u_set_imp_contratto (st_tab_contratti_rd kst_tab_contratti_rd);//
tab_1.tabpage_1.dw_1.setitem(1, "totale_contratto", kst_tab_contratti_rd.totale_contratto)


end subroutine

public subroutine u_set_imp_acconto (st_tab_contratti_rd kst_tab_contratti_rd);//
	tab_1.tabpage_1.dw_1.setitem(1, "acconto_imp", kst_tab_contratti_rd.acconto_imp)
...
kst_tab_contratti_rd.totale_contratto = 0
kst_tab_contratti_rd.acconto_imp = 0
...
for k_id = 1 to 10 
	...
	kst_tab_contratti_rd.flg_st_voce_1 = tab_1.tabpage_1.dw_1.getitemstring(1, "flg_st_voce_" + k_idx)
	if kst_tab_contratti_rd.flg_st_voce_1 = "N" then
	else
		kst_tab_contratti_rd.voce_prezzo_1 = tab_1.tabpage_1.dw_1.getitemnumber(1, "voce_prezzo_" + k_idx)
		if kst_tab_contratti_rd.voce_prezzo_1 <> 0 then
			kst_tab_contratti_rd.voce_qta_1 = tab_1.tabpage_1.dw_1.getitemnumber(1, "voce_qta_" + k_idx)
			if kst_tab_contratti_rd.voce_qta_1 > 0 then
				kst_tab_contratti_rd.voce_prezzo_tot_1 = kst_tab_contratti_rd.voce_qta_1 * kst_tab_contratti_rd.voce_prezzo_1 
				if kst_tab_contratti_rd.voce_prezzo_1 <> 0 then
					if k_id = 10 then  // lo SCONTO non è parte del Subtotale
					else
						k_subtot += kst_tab_contratti_rd.voce_qta_1 * kst_tab_contratti_rd.voce_prezzo_1 
					end if
				end if
			end if
		end if
	...
	if k_id = 10 then  // lo SCONTO è sulla decima riga
		kst_tab_contratti_rd.totale_contratto -= abs(kst_tab_contratti_rd.voce_prezzo_tot_1)
	else
		kst_tab_contratti_rd.totale_contratto += kst_tab_contratti_rd.voce_prezzo_tot_1
	end if
...
kst_tab_contratti_rd.acconto_perc = tab_1.tabpage_1.dw_1.getitemnumber(1, "acconto_perc")
if kst_tab_contratti_rd.acconto_perc > 0 then
	kst_tab_contratti_rd.acconto_imp = k_subtot * (kst_tab_contratti_rd.acconto_perc / 100)
end if
...
st_tab_contratti_rd kst_tab_contratti_rd
	...
	kst_tab_contratti_rd = u_calcola_tot( )
	u_set_imp_contratto(kst_tab_contratti_rd)
	u_set_imp_acconto(kst_tab_contratti_rd)
...
on w_contratti_rd.create
int iCurrent
call super::create
end on

on w_contratti_rd.destroy
call super::destroy
...
type st_ritorna from w_g_tab_3`st_ritorna within w_contratti_rd
end type

type st_ordina_lista from w_g_tab_3`st_ordina_lista within w_contratti_rd
end type

type st_aggiorna_lista from w_g_tab_3`st_aggiorna_lista within w_contratti_rd
end type

type cb_ritorna from w_g_tab_3`cb_ritorna within w_contratti_rd
integer x = 2711
integer y = 1468
end type

type st_stampa from w_g_tab_3`st_stampa within w_contratti_rd
end type

type cb_visualizza from w_g_tab_3`cb_visualizza within w_contratti_rd
integer x = 1179
integer y = 1472
end type

type cb_modifica from w_g_tab_3`cb_modifica within w_contratti_rd
integer x = 645
integer y = 1468
end type

type cb_aggiorna from w_g_tab_3`cb_aggiorna within w_contratti_rd
integer x = 1970
integer y = 1468
end type

type cb_cancella from w_g_tab_3`cb_cancella within w_contratti_rd
integer x = 2341
integer y = 1468
end type

type cb_inserisci from w_g_tab_3`cb_inserisci within w_contratti_rd
integer x = 1600
integer y = 1468
...
type tab_1 from w_g_tab_3`tab_1 within w_contratti_rd
integer y = 28
integer width = 3255
integer height = 1384
end type
...
integer width = 3173
integer height = 1220
string dataobject = "d_contratti_rd"
boolean minbox = true
boolean maxbox = true
...
//					post put_video_cliente(kst_tab_clienti)
	...
	elseif k_nome = "stato" then
			kst_tab_contratti_rd.stato = this.getitemstring( 1, "stato")
			choose case trim(data) 
				case kiuf_contratti_rd.kki_stato_riaperto 
					if kst_tab_contratti_rd.stato <> kiuf_contratti_rd.kki_stato_stampato then
				...
				case kiuf_contratti_rd.kki_stato_trasferito 
					kguo_exception.inizializza( )
					kguo_exception.setmessage("Non è possibile modificare lo STATO su TRASFERITO; usa l'apposita funzione")
					kguo_exception.post messaggio_utente()
					k_errore = 2
				case kiuf_contratti_rd.kki_stato_nuovo 
					kguo_exception.inizializza( )
					kguo_exception.setmessage("Non è possibile modificare lo STATO su NUOVO; usa 'RIAPERTO'")
					kguo_exception.post messaggio_utente()
					k_errore = 2
				case kiuf_contratti_rd.kki_stato_stampato 
					kst_tab_contratti_rd.id_contratto_rd = this.getitemnumber( row, "id_contratto_rd")
					if kst_tab_contratti_rd.id_contratto_rd > 0 then
						kiuf_contratti_rd.get_stato(kst_tab_contratti_rd)
					end if
					if kst_tab_contratti_rd.stato > " " then
					else
						kst_tab_contratti_rd.stato =  kiuf_contratti_rd.kki_stato_nuovo
					end if
					if kst_tab_contratti_rd.stato <> kiuf_contratti_rd.kki_stato_stampato then
						kguo_exception.inizializza( )
				...
				case kiuf_contratti_rd.kki_stato_accettato &
					    ,kiuf_contratti_rd.kki_stato_respinto 
					kst_tab_contratti_rd.id_contratto_rd = this.getitemnumber( row, "id_contratto_rd")
					if kst_tab_contratti_rd.id_contratto_rd > 0 then
						kiuf_contratti_rd.get_stato(kst_tab_contratti_rd)
					end if
...
choose case dwo.name
	...
	//--- piglio il codice del settore xchè la query va fatta con il codice
			kist_tab_contratti_rd.id_clie_settore = this.getitemstring(1, "id_clie_settore")
			k_rc = this.getchild(dwo.name, kdwc_x)
			if len(trim(kist_tab_contratti_rd.id_clie_settore)) > 0 then
	//--- Attivo dw diversi archivi 
				if kdwc_x.rowcount() > 1 then
					 if trim(kdwc_x.getitemstring(2, "id_clie_settore")) <> trim(kist_tab_contratti_rd.id_clie_settore) then
						kdwc_x.reset()
					end if
				end if
				if kdwc_x.rowcount() < 2 then
					kdwc_x.retrieve(kist_tab_contratti_rd.id_clie_settore )
					k_rc = kdwc_x.insertrow(1)
				end if
			else
				k_rc = kdwc_x.reset()
	...
	//--- piglio il codice del Gruppo xchè la query va fatta con il codice
			kst_tab_contratti_rd.gruppo = this.getitemnumber(1, "gruppo")
			if kst_tab_contratti_rd.gruppo > 0 then
	//--- Attivo dw diversi archivi 
				k_rc = this.getchild(dwo.name, kdwc_x)
				if kdwc_x.rowcount() > 1 then
					 if  isnull(kdwc_x.getitemnumber(2, "gruppo")) or kdwc_x.getitemnumber(2, "gruppo") <> kst_tab_contratti_rd.gruppo then
						kdwc_x.reset()
					end if
				end if
				if kdwc_x.rowcount() < 2 then
					kdwc_x.retrieve(kst_tab_contratti_rd.gruppo )
					k_rc = kdwc_x.insertrow(1)
				end if
			else
				k_rc = kdwc_x.reset()
...
//--- campo Gruppo Listino
	case "id_listino_voce_1" &
			...
			, "id_listino_voce_10"
		if ki_st_open_w.flag_modalita <> kkg_flag_modalita.visualizzazione then
			kst_tab_contratti_rd.id_listino_pregruppo = this.getitemnumber(1, "id_listino_pregruppo")
			if kst_tab_contratti_rd.id_listino_pregruppo > 0 then
				k_rc = this.getchild(dwo.name, kdwc_x)
				if kdwc_x.rowcount() > 1 then
					 if  isnull(kdwc_x.getitemnumber(2, "id_listino_pregruppo")) or kdwc_x.getitemnumber(2, "id_listino_pregruppo") <> kst_tab_contratti_rd.id_listino_pregruppo then
						kdwc_x.reset()
					end if
				else
					this.getchild("id_listino_voce_1", kdwc_x)
					kdwc_x.settransobject(sqlca)
					kdwc_x.retrieve(kst_tab_contratti_rd.id_listino_pregruppo )
					k_rc = kdwc_x.insertrow(1)
				end if
			else
				k_rc = kdwc_x.reset()
...
type st_duplica from w_g_tab_3`st_duplica within w_contratti_rd
end type
