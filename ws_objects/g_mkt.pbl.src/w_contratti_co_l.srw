$PBExportHeader$w_contratti_co_l.srw
forward
...
if k_riga > 0 then
	...
	kst_tab_contratti_co.id_contratto_co = dw_lista_0.getitemnumber(k_riga, "id_contratto_co")
	kst_tab_contratti_co.offerta_data = dw_lista_0.getitemdate(k_riga, "offerta_data")
...
//=== Richiesta di conferma della eliminazione del rek
	if messagebox("Elimina Documento", "Sei sicuro di voler Cancellare il documento: ~n~r" &
	         + "nr. " + string(kst_tab_contratti_co.id_contratto_co, "#####") + " del " + string(kst_tab_contratti_co.offerta_data) &
	         + " cliente " + string(kst_tab_contratti_co.id_cliente)  + " - " + trim(kst_tab_clienti.rag_soc_10),  &
...
//		ki_menu.m_strumenti.m_fin_gest_libero4.toolbaritemName = kGuo_path.get_risorse() + "\printer16x16.gif"
	...
	if not ki_menu.m_strumenti.m_fin_gest_libero7.visible then
		ki_menu.m_strumenti.m_fin_gest_libero7.libero1.text = "Mostra Contratti Commerciali in Vigore "
		...
		ki_menu.m_strumenti.m_fin_gest_libero7.libero2.text = "Mostra Contratti Commerciali Annullati"
		...
		ki_menu.m_strumenti.m_fin_gest_libero7.libero3.text = "Mostra Tutti i Contratti Commerciali"
...
try 
	kst_tab_contratti_co[] = popola_array_da_lista() // popola array con i doc selezionati per fare la stampa
	if upperbound(kst_tab_contratti_co) > 0 then

		for k_idx = 1 to upperbound(kst_tab_contratti_co)
	
			if not isnull(kst_tab_contratti_co) and kst_tab_contratti_co[k_idx].id_contratto_co > 0 then
			
				if dw_stampa.object.rb_stampa[1] = 0 then
					
					kiuf_contratti_co.stampa_documento_prova( kst_tab_contratti_co [k_idx])
				else
					
					kiuf_contratti_co.stampa_documento_definitiva(  kst_tab_contratti_co [k_idx])
				end if
...
//--- se selezionata la metto da stampare
	if dw_lista_0.IsSelected ( k_riga)   then
		...
		if dw_lista_0.getitemnumber(k_riga, "id_contratto_co") > 0 then

			k_riga_ds ++
			kst_tab_contratti_co[k_riga_ds].id_contratto_co = dw_lista_0.getitemnumber(k_riga, "id_contratto_co")
...
//--- calcolo il nr. dei contratti storici x cliente 			
	...
	kds.dataobject = "d_contratti_co_nr_x_cliente"
...
public subroutine call_crea_listino ();//
//
kuf_contratti_co_to_lis kuf1_contratti_co
...
//=== key= vedi sotto
	...
	kuf1_contratti_co = create kuf_contratti_co_to_lis
	
	K_st_open_w.flag_modalita = kkg_flag_modalita.elenco
	K_st_open_w.id_programma = kuf1_contratti_co.get_id_programma( K_st_open_w.flag_modalita ) 
...
type st_ritorna from w_g_tab0`st_ritorna within w_contratti_co_l
end type

type st_ordina_lista from w_g_tab0`st_ordina_lista within w_contratti_co_l
integer x = 1234
integer y = 1376
end type

type st_aggiorna_lista from w_g_tab0`st_aggiorna_lista within w_contratti_co_l
end type

type cb_ritorna from w_g_tab0`cb_ritorna within w_contratti_co_l
integer x = 2427
integer y = 1160
integer height = 92
integer taborder = 90
...
type st_stampa from w_g_tab0`st_stampa within w_contratti_co_l
end type

type cb_visualizza from w_g_tab0`cb_visualizza within w_contratti_co_l
integer x = 1801
integer y = 1252
end type
...
//=== dati particolare programma
	...
	K_st_open_w.id_programma = kiuf_contratti_co.get_id_programma( kkg_flag_modalita.visualizzazione )
	K_st_open_w.flag_primo_giro = "S"
	...
	K_st_open_w.key2 = "0" // trim(string(kst_tab_contratti_co.id_cliente)) // cod cliente
	K_st_open_w.key3 = " " 
	K_st_open_w.flag_where = " "
...
type cb_modifica from w_g_tab0`cb_modifica within w_contratti_co_l
integer x = 1783
integer y = 1152
integer height = 96
integer taborder = 70
...
//=== struttura st_open_w
//=== dati particolare programma
	K_st_open_w.id_programma = kiuf_contratti_co.get_id_programma( kkg_flag_modalita.modifica )
	K_st_open_w.flag_primo_giro = "S"
...
type cb_aggiorna from w_g_tab0`cb_aggiorna within w_contratti_co_l
integer x = 1134
integer y = 1160
integer height = 96
...
type cb_cancella from w_g_tab0`cb_cancella within w_contratti_co_l
integer x = 2121
integer y = 1164
integer height = 96
integer taborder = 80
...
type cb_inserisci from w_g_tab0`cb_inserisci within w_contratti_co_l
integer x = 1467
integer y = 1156
integer height = 96
integer taborder = 60
...
//=== dati particolare programma
	...
	K_st_open_w.id_programma = kiuf_contratti_co.get_id_programma( kkg_flag_modalita.inserimento )
	K_st_open_w.flag_primo_giro = "S"
...
type dw_dett_0 from w_g_tab0`dw_dett_0 within w_contratti_co_l
integer x = 1769
integer y = 1104
integer width = 827
integer height = 524
...
type st_orizzontal from w_g_tab0`st_orizzontal within w_contratti_co_l
integer x = 0
integer y = 1824
end type

type dw_lista_0 from w_g_tab0`dw_lista_0 within w_contratti_co_l
integer width = 2807
integer height = 708
integer taborder = 120
string dataobject = "d_contratti_co_l"
borderstyle borderstyle = stylelowered!
end type

type dw_guida from w_g_tab0`dw_guida within w_contratti_co_l
event ue_itemchanged ( string k_nome,  string k_dato )
boolean enabled = true
string dataobject = "d_clienti_contratti_co_guida"
end type
