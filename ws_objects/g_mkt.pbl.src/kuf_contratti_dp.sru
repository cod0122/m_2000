...
st_esito kst_esito
   ...
   SELECT  offerta_data, max(id_contratto_dp)  
	...
	if sqlca.sqlcode < 0 then
		kst_esito.esito = kkg_esito.db_ko
		kst_esito.sqlcode = sqlca.sqlcode
		kst_esito.SQLErrText = "Errore in Lettura Contratto  Conto Deposito (cercato Ultimo Documento) ~n~r:" + trim(sqlca.SQLErrText)
	end if
...
ds_contratti_dp_select kds_contratti_dp_select
	...
	if kst_tab_contratti_dp.id_contratto_dp > 0 then
		...
		if kds_contratti_dp_select.kist_errori_gestione.sqlca.sqlcode < 0 then
			kst_esito.esito = kkg_esito.db_ko
			kst_esito.sqlcode = kds_contratti_dp_select.kist_errori_gestione.sqlca.sqlcode
			kst_esito.SQLErrText = "Errore in Lettura Contratto Conto Deposito ~n~r:" + trim(kds_contratti_dp_select.kist_errori_gestione.sqlca.SQLErrText)
		end if
