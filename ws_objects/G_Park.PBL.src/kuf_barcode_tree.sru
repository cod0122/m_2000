...
public function integer u_tree_riempi_listview_x_rifer (ref kuf_treeview kuf1_treeview, readonly string k_tipo_oggetto)
public function integer u_tree_riempi_treeview_x_data (ref kuf_treeview kuf1_treeview, readonly string k_tipo_oggetto)
public function integer u_tree_riempi_treeview_da_stamp (ref kuf_treeview kuf1_treeview, readonly string k_tipo_oggetto)
...
//				kuf1_treeview.kilv_lv1.SmallPictureMaskColor = RGB(255, 255, 0)
				...
				if kst_tab_barcode.causale = ki_causale_non_trattare then
					...
					if kst_tab_barcode.data_stampa >= date(0) then
						k_stato_barcode +=  "gia' stampato il " + string(kst_tab_barcode.data_stampa, "dd.mm.yy") + "  "
					else
				...
				kuf1_treeview.kilv_lv1.setitem(k_ctr, 3, string(kst_tab_armo.alt_2, "###0") &
				                  + "x" + string(kst_tab_armo.lung_2, "###0")  &
				                  + "x" + string(kst_tab_armo.larg_2, "###0"))
				
				if isnull(kst_tab_armo.dose) then kst_tab_armo.dose = 0
				kuf1_treeview.kilv_lv1.setitem(k_ctr, 4, string(kst_tab_armo.dose, "#,##0.00"))
				...
				kuf1_treeview.kilv_lv1.setitem(k_ctr, 6, string(kst_tab_sl_pt.fila_1, "##0") + " - " &
				...
				kuf1_treeview.kilv_lv1.setitem(k_ctr, 7, string(kst_tab_sl_pt.fila_2, "##0") + " - " &
				...
				if len(trim(kst_tab_sl_pt.cod_sl_pt)) > 0 then
					...
					kuf1_treeview.kilv_lv1.setitem(k_ctr, 8, trim(kst_tab_sl_pt.cod_sl_pt) &
										 + " " + trim(kst_tab_sl_pt.descr))
				else
					kuf1_treeview.kilv_lv1.setitem(k_ctr, 8, "---")
				end if

				kuf1_treeview.kilv_lv1.setitem(k_ctr, 9, string(kst_tab_barcode.data_int , "dd/mm/yy") + string(kst_tab_barcode.num_int , "  ####0"))
...
//=== Costruisce e Dimensiona le colonne all'interno della listview
		...
		do while k_handle_item > 0
			...
			if kst_tab_barcode.data_stampa > date(0) then
				if kst_tab_barcode.pl_barcode > 0 then
					kuf1_treeview.kilv_lv1.setitem(k_ctr, k_riga_item, "p.l.:"  &
			...
			if isnull(kst_tab_sl_pt.fila_1p) then kst_tab_sl_pt.fila_1p = 0
			k_riga_item ++		
			kuf1_treeview.kilv_lv1.setitem(k_ctr, k_riga_item, string(kst_tab_sl_pt.fila_1, "##0") + " - " &
			                          + string(kst_tab_sl_pt.fila_1p, "##0"))
											  
			if isnull(kst_tab_sl_pt.fila_2) then kst_tab_sl_pt.fila_2 = 0
			if isnull(kst_tab_sl_pt.fila_2p) then kst_tab_sl_pt.fila_2p = 0
			k_riga_item ++		
			kuf1_treeview.kilv_lv1.setitem(k_ctr, k_riga_item, string(kst_tab_sl_pt.fila_2, "##0") + " - " &
			                          + string(kst_tab_sl_pt.fila_2p, "##0"))
			
			k_riga_item ++		
			if len(trim(kst_tab_sl_pt.cod_sl_pt)) > 0 then
				if isnull(kst_tab_sl_pt.descr) then kst_tab_sl_pt.descr = " "
				kuf1_treeview.kilv_lv1.setitem(k_ctr, k_riga_item, trim(kst_tab_sl_pt.cod_sl_pt) &
									 + "  " + trim(kst_tab_sl_pt.descr))
			else
...
//      if k_tipo_oggetto_padre = kuf1_treeview.kist_treeview_oggetto.pl_barcode_meca then
//			kst_treeview_data_any = kst_treeview_data.struttura
...
//			+ " count (*), " &
		k_query_select = &
			...
			k_query_where = " where " &
					...
					+ " and (barcode.causale <> '"+ kuf1_barcode.ki_causale_non_trattare +"' or barcode.causale is null)" &
