...
public function long get_wm_pklist_web (ref st_wm_pkl_web kst_wm_pkl_web_file, ref st_wm_pkl_web kst_wm_pkl_web_par[]) throws uo_exception
public function long set_wm_pklist_web_to_wm_tab (ref st_wm_pkl_web kst_wm_pkl_web[], ref st_tab_wm_receiptgammarad kst_tab_wm_receiptgammarad[]) throws uo_exception
...
//--- piglia l'elenco dei file xml contenuti nella cartella
		k_nr_file_dirlist = kuf1_file_explorer.DirList(kst_tab_wm_pklist_cfg.cartella_pkl_da_web+"\*.xml", k_file_dirlist[])

		for k_file_ind = 1 to k_nr_file_dirlist
		
//--- estrae il file da importare		
			k_file = trim(k_file_dirlist[k_file_ind])
			
//			k_file_wm_pklist_web[k_file_ind] = kst_tab_wm_pklist_cfg.cartella_pkl_da_web + "\" + k_file
			kst_wm_pkl_web[k_file_ind].nome_file = k_file
...
//---	inp: kst_wm_pkl_web_file: nome del file contenenete il pkl
...
long k_return=0
string k_rc
int k_rcn, k_file_ind
long k_ind
datastore ksd_wm_pkl_web
st_esito kst_esito
st_tab_wm_pklist_cfg kst_tab_wm_pklist_cfg
st_wm_pkl_web kst_wm_pkl_web
...
kuf_contratti kuf1_contratti
	...
	try
		...
		ksd_wm_pkl_web = create datastore

//--- piglia il path di dove sono i Packing-list Web
		kuf1_wm_pklist_cfg.get_wm_pklist_cfg(kst_tab_wm_pklist_cfg)

//--- imposta il DW e il modello di importazione del XML nel dw
		ksd_wm_pkl_web.dataobject = "d_wm_xml_pkl_web"
		k_rc = ksd_wm_pkl_web.Modify("DataWindow.Import.XML.UseTemplate = 'wm_xml_pkl_web' ")
		
		
//--- estrae il file da importare		
		k_rcn = ksd_wm_pkl_web.ImportFile(XML!, kst_wm_pkl_web_file.path_file + "\" + kst_wm_pkl_web_file.nome_file) 
		
//--- popolo le righe del st_wm_pkl_web
		for k_ind = 1 to ksd_wm_pkl_web.rowcount()
					
			k_return ++
					
			kst_wm_pkl_web.nome_file = kst_wm_pkl_web_file.nome_file
			kst_wm_pkl_web.path_file = kst_wm_pkl_web_file.path_file
			kst_wm_pkl_web.idpkl = ksd_wm_pkl_web.getitemnumber(1, "id_pkl")				
			kst_wm_pkl_web.data_invio = ksd_wm_pkl_web.getitemstring(1, "data_invio")				
			kst_wm_pkl_web.ora_invio = ksd_wm_pkl_web.getitemstring(1, "ora_invio")				
			kst_wm_pkl_web.id_cliente = ksd_wm_pkl_web.getitemnumber(1, "id_cliente")				
			kst_wm_pkl_web.mandante = trim(ksd_wm_pkl_web.getitemstring(1, "mandante"))
			kst_wm_pkl_web.ricevente = trim(ksd_wm_pkl_web.getitemstring(1, "ricevente_merce"))
			kst_wm_pkl_web.fatturato = trim(ksd_wm_pkl_web.getitemstring(1, "ricevente_fattura"))
			kst_wm_pkl_web.colliddt = ksd_wm_pkl_web.getitemnumber(1, "nr_tot_colli")
			kst_wm_pkl_web.nrddt = trim(ksd_wm_pkl_web.getitemstring(1, "nr_ddt"))
			kst_wm_pkl_web.dtddt = trim(ksd_wm_pkl_web.getitemstring(1, "data_ddt"))
			kst_wm_pkl_web.id_contratto = trim(ksd_wm_pkl_web.getitemstring(1, "id_contratto"))
			kst_wm_pkl_web.mc_co = trim(ksd_wm_pkl_web.getitemstring(1, "contratto_commerciale"))
			kst_wm_pkl_web.sc_cf = trim(ksd_wm_pkl_web.getitemstring(1, "capitolato_fornitura"))
			kst_wm_pkl_web.note_lotto = trim(ksd_wm_pkl_web.getitemstring(1, "codice_lotto"))
			kst_wm_pkl_web.barcode = trim(ksd_wm_pkl_web.getitemstring(k_ind, "barcode"))
			kst_wm_pkl_web.nr_barcode = ksd_wm_pkl_web.getitemnumber(k_ind, "nr_barcode")
...
//--- riempio l'array da ritornare
			kst_wm_pkl_web_par[k_ind] = kst_wm_pkl_web
...
//--- converte la data di invio
					if len(trim(kst_wm_pkl_web[k_file_ind].data_invio)) > 0 then
						...
						kst_tab_wm_receiptgammarad[k_ind].transmissiondate =  datetime(date(today()),time(now()))
					end if
	
					kst_tab_wm_receiptgammarad[k_ind].receiptdate	= datetime(date(today()),time(now()))
					kst_tab_wm_receiptgammarad[k_ind].ddtcode	= kst_wm_pkl_web[k_file_ind].nrddt
					...
					kst_tab_wm_receiptgammarad[k_ind].specification = kst_wm_pkl_web[k_file_ind].sc_cf
					kst_tab_wm_receiptgammarad[k_ind].contract	= kst_wm_pkl_web[k_file_ind].mc_co
					kst_tab_wm_receiptgammarad[k_ind].externalpalletcode = kst_wm_pkl_web[k_file_ind].barcode
					...
					if len(trim(kst_wm_pkl_web[k_file_ind].codice_lotto )) > 0 then
						kst_tab_wm_receiptgammarad[k_ind].customerlot = trim(kst_wm_pkl_web[k_file_ind].codice_lotto )
					else
						kst_tab_wm_receiptgammarad[k_ind].customerlot = kst_tab_wm_receiptgammarad[k_ind].packinglistcode
					end if
					kst_tab_wm_receiptgammarad[k_ind].orderdate = datetime(date(today()),time(now()))
...
string k_rc, k_nome_tag
int k_rcn, k_file_ind
long k_ind, k_nr_tag, k_ind_liv1, k_ind_liv2, k_nr_tag_liv1, k_nr_tag_liv2
st_esito kst_esito
st_tab_wm_pklist_cfg kst_tab_wm_pklist_cfg
st_wm_pkl_web kst_wm_pkl_web
st_tab_contratti kst_tab_contratti
...
//---Crea il modello di importazione del XML e imposta il file da importare
		...
		kuf1_wm_pbdom_xml_pkl_web.set_xml( kst_wm_pkl_web_file.path_file + "\" + kst_wm_pkl_web_file.nome_file) 
...
//--- piglio il nome del tag
			k_nome_tag = kuf1_wm_pbdom_xml_pkl_web.get_tag_root(k_ind)
...
//--- Questa è la root poi deve fare tutti i TAG a livello 1
				case "pkl"
//--- puglio l'attributo
					kst_wm_pkl_web.idpkl = long(kuf1_wm_pbdom_xml_pkl_web.get_attributo("id", k_ind))
...
//--- Ciclo Livello 1
			...
			//				case "note_utente"
							...
							case "contratto_commerciale"
								kst_wm_pkl_web.mc_co = kuf1_wm_pbdom_xml_pkl_web.get_valore_liv1(k_ind_liv1)
							case "capitolato_fornitura"
								kst_wm_pkl_web.sc_cf = kuf1_wm_pbdom_xml_pkl_web.get_valore_liv1(k_ind_liv1)
							case "nr_ddt"
							...
							case "ricevente_merce"
								kst_wm_pkl_web.ricevente = kuf1_wm_pbdom_xml_pkl_web.get_valore_liv1(k_ind_liv1)
							case "ricevente_fattura"
								kst_wm_pkl_web.fatturato = kuf1_wm_pbdom_xml_pkl_web.get_valore_liv1(k_ind_liv1) 
							case "codice_lotto"
							...
							case "no_etichette"
									kst_wm_pkl_web.barcode = ""
									kst_wm_pkl_web.nr_barcode = 0

//--- Riempie dati generici UGUALE A SOTTO ----------------------------
									if isnull(kst_wm_pkl_web.mandante) or len(kst_wm_pkl_web.mandante) = 0 then
										if kst_wm_pkl_web.id_cliente > 0 then
											kst_wm_pkl_web.mandante = string(kst_wm_pkl_web.id_cliente)
										else
...
//--- Ciclo Livello 2
								...
								for k_ind_liv2 = 1 to k_nr_tag_liv2
									...
									choose case lower(k_nome_tag)
										...
										case "barcode"
											kst_wm_pkl_web.barcode = kuf1_wm_pbdom_xml_pkl_web.get_valore_liv2(k_ind_liv2) 
										case "nr_barcode"
											if isnumber(trim(kuf1_wm_pbdom_xml_pkl_web.get_valore_liv2(k_ind_liv2) )) then
												kst_wm_pkl_web.nr_barcode = long(kuf1_wm_pbdom_xml_pkl_web.get_valore_liv2(k_ind_liv2) )
											end if
											
									end choose

//--- Riempie dati generici UGUALE A SOPRA ----------------------------
