...
public subroutine importa_wm_pklist (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
private subroutine importa_wm_pklist_esegui (ref st_tab_wm_pklist kst_tab_wm_pklist) throws uo_exception
...
public subroutine memo (st_tab_meca_memo ast_tab_meca_memo) throws uo_exception
...
//--- Se NUM_INT cambiato rispetto a quello passato allora il vecchio lo salvo nel campo CONTATI
		...
		if isnull(kist_tab_meca.data_int) or kist_tab_meca.data_int = date(0) then kist_tab_meca.data_int = kst_tab_meca_appo.data_int

//--- carico Testa Lotto 		
		kst_esito = kiuf_armo.tb_update_meca( kist_tab_meca )
...
//--- carico Causale Lotto 	
		if kist_tab_meca.id_meca_causale > 0 then
			kst_esito = kiuf_armo.tb_update_meca_blk(kist_tab_meca)
			if kst_esito.esito <> kkg_esito_ok then 
				kuo_exception = create uo_exception
				kuo_exception.set_esito(kst_esito)
				throw kuo_exception
...
//			do while kist_tab_armo[k_ctr].id_armo > 0 
		...
		kst_esito = kiuf_armo.tb_update_armo( kist_tab_armo )
		if kst_esito.esito <> kkg_esito_ok then 
			kuo_exception = create uo_exception
			kuo_exception.set_esito(kst_esito)
			throw kuo_exception
		...
		k_return = kist_tab_armo.id_armo
...
//--- se OK allora IMPORTA!			
			importa_wm_pklist_esegui(kst_tab_wm_pklist)
...
//--- Get del Ricevente e Fatturato
			...
			if kst_tab_m_r_f.clie_3 > 0 then
				...
				kuf1_clienti.get_mrf_clie_3(kst_tab_m_r_f)
				if kst_esito.esito <> kkg_esito_ok and  kst_esito.esito <> kkg_esito_db_wrn and kst_esito.esito <> kkg_esito_not_fnd then
					kguo_exception.set_esito(kst_esito)
					throw kguo_exception
				end if
...
//--- Get del Contratto
			if (len(trim(kst_tab_wm_pklist.sc_cf)) > 0 or len(trim(kst_tab_wm_pklist.mc_co)) > 0) and kst_tab_wm_pklist.clie_3 > 0 then
				...
				kuf1_contratti.get_codice_da_cf_co(kst_tab_contratti )
			end if
...
//--- Get del eventuale Causale di Blocco impostata da Contratto
			...
			kist_tab_meca.stato = kiuf_armo.ki_meca_stato_gen_da_pkl 
			if kist_tab_meca.id_meca_causale > 0 then 
...
//--- aggiorna lo stato della Packing		
			...
			if kst_esito.esito <> kkg_esito_ok and  kst_esito.esito <> kkg_esito_db_wrn then
				kuf1_data_base.db_rollback_1( ) //--- Rollback delle Modifiche
				kguo_exception.set_esito(kst_esito)
				throw kguo_exception
			end if
...
//--- Trovo il primo Articolo che è OK	
					k_ind_listino = 1
					kst_esito = kuf1_listino.get_x_riga_lotto(kst_tab_listino[k_ind_listino])
					if kst_esito.esito <> kkg_esito_ok and  kst_esito.esito <> kkg_esito_db_wrn and  kst_esito.esito <> kkg_esito_not_fnd then
						kuf1_data_base.db_rollback_1( ) //--- Rollback delle Modifiche
						kguo_exception.set_esito(kst_esito)
						throw kguo_exception
...
//--- se in packing ho indicato il GRUPPO allora deve metchare 
					if kst_tab_wm_pklist_righe[k_ctr_righe_pklist].gruppo > 0 then
						do while kst_tab_prodotti.gruppo <> k_gruppo and  kst_esito.esito <> kkg_esito_not_fnd
...
//--- Get del Codice Piano di Trattamento						
					...
					if kst_esito.esito <> kkg_esito_ok then kst_tab_contratti.sl_pt = ""
...
//--- Get Descrizione Prodotto 
					...
					kist_tab_armo.pedane = kist_tab_armo.colli_2   
...
//--- Get delle note lotto da caricare nelle NOTE dell'entrata (SOLO sulla prima riga)
					...
					if k_ind_riga_armo = 1 then
						...
						if len(trim(kst_tab_wm_pklist.note_lotto)) > 0 then
							...
							kst_tab_armo_nt.note[10] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 9 + 1))
							kst_tab_armo_nt.note[9] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 8 + 1),53)
							kst_tab_armo_nt.note[8] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 7 + 1),53)
							kst_tab_armo_nt.note[7] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 6 + 1),53)
							kst_tab_armo_nt.note[6] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 5 + 1),53)
							kst_tab_armo_nt.note[5] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 4 + 1),53)
							kst_tab_armo_nt.note[4] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 3 + 1),53)
							kst_tab_armo_nt.note[3] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 2 + 1),53)
							kst_tab_armo_nt.note[2] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 1 + 1),53)
							kst_tab_armo_nt.note[1] = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 3 + 53 * 0 + 1),53)
							kist_tab_armo.note_3 = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 2 + 1),51)
							kist_tab_armo.note_2 = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 1 + 1),51)
							kist_tab_armo.note_1 = mid(trim(kst_tab_wm_pklist.note_lotto ),(51 * 0 + 1),51)
						else
...
//--- Se ho delle note da scrivere lo faccio subito
					if len(trim(kst_tab_armo_nt.note[1])) > 0 then
						...
						kst_esito_armo_nt = kiuf_armo.tb_add_armo_nt(kst_tab_armo_nt)  // Insert delle note
						if kst_esito_armo_nt.esito <> kkg_esito_ok then
							kguo_exception.set_esito(kst_esito_armo_nt)
						end if
...
//--- controlla se ha gia' una PKL associata						
			kuf1_armo = create kuf_armo
			kst_esito = kuf1_armo.get_id_wm_pklist(kst_tab_meca) 
			if kst_esito.esito <> kkg_esito_ok then 
				kuo_exception = create uo_exception
				kuo_exception.set_esito(kst_esito)
				throw kuo_exception
...
//--- se è un prezzo SPOT carico subito i colli da fatturare					
					if kds_listino_voci_pregruppo_prezzo.getitemstring(k_ind_prezzo, "tipo_calcolo") = kuf1_listino_voci.kki_tipo_calcolo_spot then
...
try   
	...
	if ast_tab_meca_memo.id_meca  > 0 then
		...
		k_settore = ""
		kst_tab_memo.tipo_sv = ast_tab_meca_memo.tipo_sv
		...
		if ast_tab_meca_memo.tipo_sv > " " then
			kst_tab_memo.titolo = "Informazioni dal dip. '" + ast_tab_meca_memo.tipo_sv + "' per il Lotto " + string(kst_tab_meca.num_int) + " / " + string(kst_tab_meca.data_int)
		else
			kst_tab_memo.titolo = "Informazioni  per il Lotto " + string(kst_tab_meca.num_int) + " / " + string(kst_tab_meca.data_int)
		end if
		if k_settore > " " then
			kst_tab_memo.note = "Note dal dip. '" + k_settore + "' per il Lotto id " + string(kst_tab_meca.id) + " nr. " + string(kst_tab_meca.num_int) + " del " + string(kst_tab_meca.data_int)
		else
			kst_tab_memo.note = "Note per il Lotto id " + string(kst_tab_meca.id) + " nr. " + string(kst_tab_meca.num_int) + " del " + string(kst_tab_meca.data_int)
		end if
		
//		kst_tab_memo.tipo_sv = ast_tab_meca_memo.tipo_sv
//		if ast_tab_meca_memo.tipo_sv = kki_tipo_sv_MAG then
//			kst_tab_memo.titolo = "Informazioni Lotto " + string(kst_tab_meca.num_int) + " / " + string(kst_tab_meca.data_int)
//			kst_tab_memo.note = "Note Lotto id " + string(kst_tab_meca.id) + " nr. " + string(kst_tab_meca.num_int) + " del " + string(kst_tab_meca.data_int)
//		else
//			kst_tab_memo.titolo = "Informazioni Lotto " + string(kst_tab_meca.num_int) + " / " + string(kst_tab_meca.data_int)
//			kst_tab_memo.note = "Note di " + kguo_utente.get_nome( ) + " per il Lotto id " + string(kst_tab_meca.id) + " nr. " + string(kst_tab_meca.num_int) + " del " + string(kst_tab_meca.data_int)
//		end if
		kst_tab_memo.id_memo = ast_tab_meca_memo.id_memo
		
		kist_tab_meca_memo = ast_tab_meca_memo
		
		if NOT isvalid(kuf1_memo) then kuf1_memo = create kuf_memo 
//		kuf1_memo.set_kuf_meca(this)
...
if ast_tab_meca_memo.id_memo > 0 then
	...
	if kguo_sqlca_db_magazzino.sqlcode <> 0 then
		if kguo_sqlca_db_magazzino.sqlcode < 0 then
			kst_esito.esito = kkg_esito_db_ko
			kst_esito.sqlcode = kguo_sqlca_db_magazzino.sqlcode
			kst_esito.SQLErrText = "Errore Cancellazione NOTE dal Lotto (id memo " + string(ast_tab_meca_memo.id_memo) + ")~n~r" + trim(kguo_sqlca_db_magazzino.SQLErrText) 
...
st_esito kst_esito
	...
	if sqlca.sqlcode < 0 then
		kst_esito.esito = kkg_esito_db_ko
		kst_esito.sqlcode = sqlca.sqlcode
		kst_esito.SQLErrText = "Errore in Lettura Memo-Lotto 2 entrato (id = " + string(kst_tab_meca_memo.ID_meca) + ") ~n~r:" + trim(sqlca.SQLErrText)
	end if
...
kst_tab_meca_memo.x_datins = kuf1_data_base.prendi_x_datins()
kst_tab_meca_memo.x_utente = kuf1_data_base.prendi_x_utente()
