...
type dw_riga from uo_d_sped_riga within w_ddt
end type
end forward
...
public subroutine u_inizializza (st_tab_sped ast_tab_sped)
...
long k_riga, k_colli
st_tab_sped kst_tab_sped, kst_tab_sped_app
...
//if tab_1.tabpage_1.dw_1.getnextmodified(0, primary!) > 0	or tab_1.tabpage_4.dw_4.getnextmodified(0, primary!) > 0 or  tab_1.tabpage_4.dw_4.deletedcount() > 0 then
	...
	//--- Se sono in Inserimento verifico se numero documento già usato
		if ki_st_open_w.flag_modalita = kkg_flag_modalita.inserimento then
			...
			kist_tab_sped_orig.num_bolla_out = kst_tab_sped_app.num_bolla_out
			kst_tab_sped_app.data_bolla_out = tab_1.tabpage_1.dw_1.getitemdate(k_riga, "data_bolla_out") 
			kst_tab_sped_app.id_sped = 0
			if kiuf_sped.get_id_sped( kst_tab_sped_app ) then
				kiuf_sped.get_ultimo_doc ( kst_tab_sped_app ) 
				kst_tab_sped.num_bolla_out = kst_tab_sped_app.num_bolla_out + 1
	
				tab_1.tabpage_1.dw_1.setitem(k_riga, "num_bolla_out", kst_tab_sped.num_bolla_out )
...
//--- se sono in Modifica e ho cambiato numero e data....			
			if ki_st_open_w.flag_modalita = kkg_flag_modalita_modifica then
				...
				if kist_tab_sped_orig.num_bolla_out > 0 then
					if kist_tab_sped_orig.num_bolla_out <> kst_tab_sped.num_bolla_out or kist_tab_sped_orig.data_bolla_out <> kst_tab_sped.data_bolla_out then
...
//---- Prima rimuove le righe da CANCELLARE DAL DB 
			k_riga = 1
			do while k_riga <= tab_1.tabpage_4.dw_4.DeletedCount ( ) and kst_esito.esito <> kkg_esito.db_ko
...
//=== Se tutto OK faccio la COMMIT		
		if kst_esito.esito = kkg_esito.db_ko then
			...
			if kst_tab_sped.num_bolla_out <> kist_tab_sped_orig.num_bolla_out then
				kguo_exception.setmessage( "Modificato il numero iniziale impostato per questo DDT")
				...
				if k_riga > 0 then
					if tab_1.tabpage_1.dw_1.getitemstatus(k_riga, 0, primary!) <> new!  &
						...
						k_record_1 = &
							...
							" del " + string(kst_tab_sped.data_bolla_out) + " ?"
					else
...
if isnull(kst_tab_sped_attuale.id_sped) then kst_tab_sped_attuale.id_sped = 0
...
//if kist_tab_sped_orig.id_sped <> kst_tab_sped_attuale.id_sped or tab_1.tabpage_1.dw_1.rowcount() = 0 then
...
//--- controlli particolari	
		if ki_st_open_w.flag_modalita = kkg_flag_modalita.modifica and k_flag_numero_ddt_modificato then
...
//			tab_1.tabpage_1.dw_1.setitem( 1, "ora_rit", string(kguo_g.get_ora( )) + ":" + string(kguo_g.get_minuti( )) )
			...
			kist_tab_sped.num_bolla_out = kiuf_sped.get_numero_nuovo()
...
//				and tab_1.tabpage_4.enabled then
		...
		if kI_menu.m_strumenti.m_fin_gest_libero1.enabled <> k_attiva or ki_st_open_w.flag_primo_giro = "S" then 
			...
			kI_menu.m_strumenti.m_fin_gest_libero1.toolbaritemname = kG_path_risorse + kkg.path_sep + "lotto16x16.gif"
		end if
		...
		if kI_menu.m_strumenti.m_fin_gest_libero2.enabled <> k_attiva or ki_st_open_w.flag_primo_giro = "S" then 
			...
			kI_menu.m_strumenti.m_fin_gest_libero2.toolbaritemname = kG_path_risorse + kkg.path_sep + "lotto16x16_red.gif"
		end if

		if ki_st_open_w.flag_modalita <> kkg_flag_modalita_visualizzazione &
				and ki_st_open_w.flag_modalita <> kkg_flag_modalita_cancellazione then
			k_attiva = true
		else
			k_attiva = false
		end if
		if kI_menu.m_strumenti.m_fin_gest_libero3.enabled <> k_attiva or ki_st_open_w.flag_primo_giro = "S" then
			...
			kI_menu.m_strumenti.m_fin_gest_libero3.toolbaritemName = kG_path_risorse + kkg.path_sep + "find16.png"
		end if
		...
		if kI_menu.m_strumenti.m_fin_gest_libero4.enabled <> k_attiva or ki_st_open_w.flag_primo_giro = "S" then
			...
			kI_menu.m_strumenti.m_fin_gest_libero4.toolbaritemName = kg_path_risorse + kkg.path_sep + "printer16x16.gif"
...
tab_1.tabpage_1.dw_1.modify( "clie_2.Background.Color = '" + string(KK_COLORE_BIANCO) + "' " ) 
...
tab_1.tabpage_1.dw_1.modify( "id_vettore.Background.Color = '" + string(KK_COLORE_BIANCO) + "' " ) 
...
tab_1.tabpage_1.dw_1.modify( "caus_codice.Background.Color = '" + string(KK_COLORE_BIANCO) + "' " ) 
...
tab_1.tabpage_1.dw_1.modify( "clie_3.Background.Color = '" + string(KK_COLORE_BIANCO) + "' " ) 
tab_1.tabpage_1.dw_1.setitem( 1, "clie_3", kst_tab_clienti.codice )
tab_1.tabpage_1.dw_1.modify( "clie_3_rag_soc.Background.Color = '" + string(KK_COLORE_BIANCO) + "' " ) 
...
public subroutine u_inizializza (st_tab_sped ast_tab_sped);//
	try
		...
		if kiuf_sped.get_id_sped(ast_tab_sped) then
...
//--- se cliente già indicato non lo prendo	
		if kst_tab_meca.clie_2 > 0 then
			...
			if kst_tab_meca_1.clie_2 > 0 and kst_tab_meca_1.clie_2 <> kst_tab_meca.clie_2 then
				kguo_exception.inizializza( )
				kguo_exception.set_tipo(kguo_exception.kk_st_uo_exception_tipo_dati_non_eseguito)
				kguo_exception.setmessage("Il Lotto indicato contiene il Ricevente " + string(kst_tab_meca_1.clie_2) + " diverso da quello indicato (" + string(kst_tab_meca.clie_2) + ") !")
				throw kguo_exception
			end if
		else
...
try
	choose case dwo.name 
		...
		case "num_bolla_out", "data_bolla_out"
			...
			post u_inizializza(kst_tab_sped)
