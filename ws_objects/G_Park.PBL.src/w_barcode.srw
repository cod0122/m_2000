...
//=== Nr righe ne DW lista
	choose case tab_1.selectedtab
		case 1
			...
			if k_riga > 0 then
				k_codice = trim(tab_1.tabpage_1.dw_1.getitemstring(k_riga, "barcode_barcode"))
			end if
			if k_riga <= 0 or len(k_codice) = 0 or isnull(k_codice) then
				cb_inserisci.enabled = false
				cb_inserisci.default = false
				cb_cancella.enabled = false
				cb_aggiorna.enabled = false
...
//=== Salvo ID del rec originale x piu' avanti
	k_codice_1 = tab_1.tabpage_1.dw_1.getitemstring(1, "barcode_barcode")
...
//=== Se non sono in caricamento allora prelevo l'ID dalla dw
	if tab_1.tabpage_1.dw_1.getitemstatus(1, 0, primary!) <> newmodified! then
		k_codice = tab_1.tabpage_1.dw_1.getitemstring(1, "barcode_barcode")
	else
...
//		tab_1.tabpage_1.dw_1.setitem(1, "cod_sl_pt", k_codice)
...
//=== Controllo il primo tab
	...
	k_key = tab_1.tabpage_1.dw_1.getitemstring ( k_riga, "barcode_barcode") 
	if isnull(k_key) or len(trim(k_key)) = 0 then
		k_return = tab_1.tabpage_1.text + ": Manca il Codice a Barre! " + "~n~r"
		k_errore = "3"
		k_nr_errori++
	end if
	...
	for k_riga = k_nr_righe to 1 step -1
		if tab_1.tabpage_1.dw_1.getitemstatus(k_riga, 0, primary!) = newmodified!  &
		   or tab_1.tabpage_1.dw_1.getitemstatus(k_riga, 0, primary!) = DataModified!	 then 
			if (isnull(tab_1.tabpage_1.dw_1.getitemstring ( k_riga, "barcode_barcode")) or &
				 len(trim(tab_1.tabpage_1.dw_1.getitemstring ( k_riga, "barcode_barcode"))) = 0) &
				then
	...
	case 1 
		...
		if k_riga > 0 then
			if tab_1.tabpage_1.dw_1.getitemstatus(k_riga, 0, primary!) <> new! and &
				...
				k_key = tab_1.tabpage_1.dw_1.getitemstring(k_riga, "barcode_barcode")
				k_desc = "Riferimento: " + string(tab_1.tabpage_1.dw_1.getitemnumber(k_riga, "barcode_num_int"), "####0") &
...
//			case 2
//				k_errore = kuf1_barcode.tb_delete_ind_comm(k_key) 
		end choose	
		if left(k_errore, 1) = "0" then
			...
			case 0
				...
				if k_scelta <> "in" then
					...
					messagebox("Ricerca fallita", &
						"Mi spiace ma il BARCODE non e' in Archivio ~n~r" + &
						"(Codice ricercato:" + trim(k_key) + ")~n~r" )
...
event open;call super::open;//--- creo il datasore (dw non visuale) per appoggio elenco
