$PBExportHeader$uo_d_std_trova_old.sru
forward
global type uo_d_std_trova_old from datawindow
end type
end forward
...
global uo_d_std_trova_old uo_d_std_trova_old
...
public function boolean set_obj_trova (ref graphicobject aany_oggetto)
...
//	put_obj_trova()
//	
//	kiany_oggetto_trova = get_obj_trova() 
...
//			this.y = kiw_this_window.y +  this.height / 2 
		end if
		this.enabled = true
		this.visible=true
		this.setfocus()
...
boolean k_return = false
	...
	if not isnull(kiany_oggetto_trova) then
...
public function boolean set_obj_trova (ref graphicobject aany_oggetto);//---
//--- Imposta l'oggetto su cui fare il TROVA
//---
boolean k_return = false
...
if isvalid(aany_oggetto) then
	if TypeOf(aany_oggetto) = DataWindow! or TypeOf(aany_oggetto) = Listview! then
		...
		kguo_exception.messaggio_utente("Trova Dati", "Scegliere l'elenco su cui Cercare")
	end if
else
	setnull(kiany_oggetto_trova)
	kguo_exception.inizializza( )
	kguo_exception.set_tipo( kguo_exception.kk_st_uo_exception_tipo_dati_insufficienti )
	kguo_exception.messaggio_utente("Trova Dati", "Scegliere prima l'elenco su cui Cercare")
end if
...
//				messagebox("Ricerca fallita", "Dato richiesto non trovato in elenco")
	...
	//--- scroll della lista sull'indice trovato
				if not (kuf1_data_base.u_listview_scroll(klv_trova, k_riga)) then
					SetPointer(oldpointer)
...
on uo_d_std_trova_old.create
end on

on uo_d_std_trova_old.destroy
end on
